pipeline {
    ...
    stages {
        ...
        stage('Publish Helm Chart') {
            ...
            environment {
                IMAGE_SHA = "${image_sha}"
                HELM_PACKAGE_FILENAME = sh (script: 'echo "$(yq -r .name ./chart/Chart.yaml)-$(yq -r .version ./chart/Chart.yaml).tgz"', returnStdout: true).trim()
            }
            steps {
                withCredentials([
                usernamePassword(
                  ...
                  passwordVariable: 'REGISTRY_PASSWORD'
                    ...
                    helm package ./chart/
                    helm push "${HELM_PACKAGE_FILENAME}" "oci://registry-1.docker.io/${REGISTRY_USERNAME}"
                    '''
                }
                    archiveArtifacts artifacts: "${HELM_PACKAGE_FILENAME}", fingerprint: true
            }    
        }
