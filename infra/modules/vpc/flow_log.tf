...
resource "aws_kms_key" "this" {
  ...
  # policy = jsonencode({
  #   Id = "${aws_kms_key.this[0].key_id}-encrypt-key-policy"
  #   Statement = [
  #     {
...
resource "aws_kms_key_policy" "this" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      # {
      #   Sid    = "Enable IAM User Permissions to MyPowerUser"
      #   Effect = "Allow"
      #   Principal = {
      #     AWS = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:user/mypoweruser"   # TODO: Replace dynamically with a variable value of username of my poweruser IAM user
      #   },
      #   Action   = "kms:*"
      #   Resource = aws_kms_key.this[0].arn
      # },
